...
class BookModel extends AdminModel
{
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_getbible.book';
	...
	public function getForm($data = [], $loadData = true, $options = ['control' => 'jform'])
	{
		...
		// Get the form.
		$form = $this->loadForm('com_getbible.book', 'book', $options, $clear, $xpath);
		...
		if ($id != 0 && (!$user->authorise('book.edit.state', 'com_getbible.book.' . (int) $id))
			|| ($id == 0 && !$user->authorise('book.edit.state', 'com_getbible')))
		{
		...
		if ($id != 0 && (!$user->authorise('book.edit.created_by', 'com_getbible.book.' . (int) $id))
			|| ($id == 0 && !$user->authorise('book.edit.created_by', 'com_getbible')))
		{
			// Disable fields for display.
		...
		if ($id != 0 && (!$user->authorise('book.edit.created', 'com_getbible.book.' . (int) $id))
			|| ($id == 0 && !$user->authorise('book.edit.created', 'com_getbible')))
		{
			// Disable fields for display.
		...
		// Modify the form based on Edit Nr access controls.
	...
	protected function canDelete($record)
	{
		...
		return $this->getCurrentUser()->authorise('book.delete', 'com_getbible.book.' . (int) $record->id);
	}
	...
	protected function canEditState($record)
	{
		...
		{
			// The record has been set. Check the record permissions.
			$permission = $user->authorise('book.edit.state', 'com_getbible.book.' . (int) $recordId);
	...
	protected function allowEdit(array $data = [], string $key = 'id'): bool
	{
		...
		{
			// The record has been set. Check the record permissions.
			$permission = $user->authorise('book.edit', 'com_getbible.book.' . (int) $recordId);
			if (!$permission)
			{
				if ($user->authorise('book.edit.own', 'com_getbible.book.' . $recordId))
				{
					...
					if ($ownerId == $user->id)
					{
						if ($user->authorise('book.edit.own', 'com_getbible'))
						{
							return true;
						}
					}
		...
		return $user->authorise('book.edit', $this->option);
	}
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__getbible_book'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_getbible.edit.book.data', []);
		...
		$this->preprocessData('com_getbible.book', $data);
	...
	protected function batchCopy($values, $pks, $contexts)
	{
		if (empty($this->batchSet))
		{
			...
			$this->table 		= $this->getTable();
			$this->tableClassName	= get_class($this->table);
			$this->canDo		= Actions::get('book');
		}

		if (!$this->canDo->get('book.create') && !$this->canDo->get('book.batch'))
		{
			return false;
		}
		...
		elseif (isset($values['published']) && !$this->canDo->get('book.edit.state'))
		{
				$values['published'] = 0;
		}
		...
		while (!empty($pks))
		{
			...
			if (!$this->user->authorise('book.edit', $contexts[$pk]))
			{
				// Not fatal error
	...
	protected function batchMove($values, $pks, $contexts)
	{
		...
		if (!$this->canDo->get('book.edit') && !$this->canDo->get('book.batch'))
		{
		...
		if (isset($values['published']) && !$this->canDo->get('book.edit.state'))
		{
