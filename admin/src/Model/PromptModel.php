...
class PromptModel extends AdminModel
{
	 ...
	 * @throws  \Exception
	 */
	public function getTable($type = 'prompt', $prefix = 'Administrator', $config = [])
	{
	...
	protected function canDelete($record)
	{
		...
		return $this->getCurrentUser()->authorise('prompt.delete', 'com_getbible.prompt.' . (int) $record->id);
	}
	...
	protected function allowEdit(array $data = [], string $key = 'id'): bool
	{
		...
		{
			// The record has been set. Check the record permissions.
			$permission = $user->authorise('prompt.edit', 'com_getbible.prompt.' . (int) $recordId);
			if (!$permission)
			{
				if ($user->authorise('prompt.edit.own', 'com_getbible.prompt.' . $recordId))
				{
					...
					if ($ownerId == $user->id)
					{
						if ($user->authorise('prompt.edit.own', 'com_getbible'))
						{
							return true;
						}
					}
		...
		return $user->authorise('prompt.edit', $this->option);
	}
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_getbible.edit.prompt.data', []);
	...
	public function save($data)
	{
		...
		elseif (!isset($data['messages'])
			&& Factory::getApplication()->getIdentity()->authorise('prompt.edit.messages', 'com_getbible'))
		{
