...
class Tagged_verseModel extends AdminModel
{
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_getbible.tagged_verse';
	 ...
	 * @throws  \Exception
	 */
	public function getTable($type = 'tagged_verse', $prefix = 'Administrator', $config = [])
	{
	...
	public function getForm($data = [], $loadData = true, $options = ['control' => 'jform'])
	{
		...
		// Get the form.
		$form = $this->loadForm('com_getbible.tagged_verse', 'tagged_verse', $options, $clear, $xpath);
		...
		if ($id != 0 && (!$user->authorise('tagged_verse.edit.state', 'com_getbible.tagged_verse.' . (int) $id))
			|| ($id == 0 && !$user->authorise('tagged_verse.edit.state', 'com_getbible')))
		{
		...
		if ($id != 0 && (!$user->authorise('tagged_verse.edit.created_by', 'com_getbible.tagged_verse.' . (int) $id))
			|| ($id == 0 && !$user->authorise('tagged_verse.edit.created_by', 'com_getbible')))
		{
			// Disable fields for display.
		...
		if ($id != 0 && (!$user->authorise('tagged_verse.edit.created', 'com_getbible.tagged_verse.' . (int) $id))
			|| ($id == 0 && !$user->authorise('tagged_verse.edit.created', 'com_getbible')))
		{
			// Disable fields for display.
		...
		if ($id != 0 && (!$user->authorise('tagged_verse.edit.access', 'com_getbible.tagged_verse.' . (int) $id))
			|| ($id == 0 && !$user->authorise('tagged_verse.edit.access', 'com_getbible')))
		{
			// Disable fields for display.
	...
	protected function canDelete($record)
	{
		...
		return $this->getCurrentUser()->authorise('tagged_verse.delete', 'com_getbible.tagged_verse.' . (int) $record->id);
	}
	...
	protected function canEditState($record)
	{
		...
		{
			// The record has been set. Check the record permissions.
			$permission = $user->authorise('tagged_verse.edit.state', 'com_getbible.tagged_verse.' . (int) $recordId);
	...
	protected function loadFormData()
	{
		...
		$this->preprocessData('com_getbible.tagged_verse', $data);
	...
	protected function batchCopy($values, $pks, $contexts)
	{
		...
		elseif (isset($values['published']) && !$this->canDo->get('tagged_verse.edit.state'))
		{
				$values['published'] = 0;
		}
	...
	protected function batchMove($values, $pks, $contexts)
	{
		...
		if (isset($values['published']) && !$this->canDo->get('tagged_verse.edit.state'))
		{
	...
	public function save($data)
	{
		...
		if (empty($data['guid']) && $data['id'] > 0)
		{
			...
			$data['guid'] = (string) GetHelper::var('tagged_verse', $data['id'], 'id', 'guid');
		}

		// Set the GUID if empty or not valid
		while (!GuidHelper::valid($data['guid'], "tagged_verse", $data['id']))
		{
			// must always be set
