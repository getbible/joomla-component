...
class ChapterModel extends AdminModel
{
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_getbible.chapter';
	 ...
	 * @throws  \Exception
	 */
	public function getTable($type = 'chapter', $prefix = 'Administrator', $config = [])
	{
	...
	public function getForm($data = [], $loadData = true, $options = ['control' => 'jform'])
	{
		...
		// Get the form.
		$form = $this->loadForm('com_getbible.chapter', 'chapter', $options, $clear, $xpath);
		...
		if ($id != 0 && (!$user->authorise('chapter.edit.state', 'com_getbible.chapter.' . (int) $id))
			|| ($id == 0 && !$user->authorise('chapter.edit.state', 'com_getbible')))
		{
		...
		if ($id != 0 && (!$user->authorise('chapter.edit.created_by', 'com_getbible.chapter.' . (int) $id))
			|| ($id == 0 && !$user->authorise('chapter.edit.created_by', 'com_getbible')))
		{
			// Disable fields for display.
		...
		if ($id != 0 && (!$user->authorise('chapter.edit.created', 'com_getbible.chapter.' . (int) $id))
			|| ($id == 0 && !$user->authorise('chapter.edit.created', 'com_getbible')))
		{
			// Disable fields for display.
	...
	protected function canDelete($record)
	{
		...
		return $this->getCurrentUser()->authorise('chapter.delete', 'com_getbible.chapter.' . (int) $record->id);
	}
	...
	protected function canEditState($record)
	{
		...
		{
			// The record has been set. Check the record permissions.
			$permission = $user->authorise('chapter.edit.state', 'com_getbible.chapter.' . (int) $recordId);
	...
	protected function loadFormData()
	{
		...
		$this->preprocessData('com_getbible.chapter', $data);
	...
	protected function batchCopy($values, $pks, $contexts)
	{
		...
		elseif (isset($values['published']) && !$this->canDo->get('chapter.edit.state'))
		{
				$values['published'] = 0;
		}
	...
	protected function batchMove($values, $pks, $contexts)
	{
		...
		if (isset($values['published']) && !$this->canDo->get('chapter.edit.state'))
		{
