...
class NoteModel extends AdminModel
{
	...
	protected function allowEdit(array $data = [], string $key = 'id'): bool
	{
		...
		{
			// The record has been set. Check the record permissions.
			$permission = $user->authorise('note.edit', 'com_getbible.note.' . (int) $recordId);
			if (!$permission)
			{
				if ($user->authorise('note.edit.own', 'com_getbible.note.' . $recordId))
				{
					...
					if ($ownerId == $user->id)
					{
						if ($user->authorise('note.edit.own', 'com_getbible'))
						{
							return true;
						}
					}
		...
		return $user->authorise('note.edit', $this->option);
	}
