...
class PasswordModel extends AdminModel
{
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_getbible.password';
	 ...
	 * @throws  \Exception
	 */
	public function getTable($type = 'password', $prefix = 'Administrator', $config = [])
	{
	...
	public function getForm($data = [], $loadData = true, $options = ['control' => 'jform'])
	{
		...
		// Get the form.
		$form = $this->loadForm('com_getbible.password', 'password', $options, $clear, $xpath);
		...
		if ($id != 0 && (!$user->authorise('password.edit.state', 'com_getbible.password.' . (int) $id))
			|| ($id == 0 && !$user->authorise('password.edit.state', 'com_getbible')))
		{
		...
		if ($id != 0 && (!$user->authorise('password.edit.created_by', 'com_getbible.password.' . (int) $id))
			|| ($id == 0 && !$user->authorise('password.edit.created_by', 'com_getbible')))
		{
			// Disable fields for display.
		...
		if ($id != 0 && (!$user->authorise('password.edit.created', 'com_getbible.password.' . (int) $id))
			|| ($id == 0 && !$user->authorise('password.edit.created', 'com_getbible')))
		{
			// Disable fields for display.
		...
		if ($id != 0 && (!$user->authorise('password.access.password', 'com_getbible.password.' . (int) $id))
			|| ($id == 0 && !$user->authorise('password.access.password', 'com_getbible')))
		{
			// Remove the field
			$form->removeField('password');
		}
		// Modify the form based on View Password access controls.
		if ($id != 0 && (!$user->authorise('password.view.password', 'com_getbible.password.' . (int) $id))
			|| ($id == 0 && !$user->authorise('password.view.password', 'com_getbible')))
		{
	...
	protected function canDelete($record)
	{
		...
		return $this->getCurrentUser()->authorise('password.delete', 'com_getbible.password.' . (int) $record->id);
	}
	...
	protected function canEditState($record)
	{
		...
		{
			// The record has been set. Check the record permissions.
			$permission = $user->authorise('password.edit.state', 'com_getbible.password.' . (int) $recordId);
	...
	protected function allowEdit(array $data = [], string $key = 'id'): bool
	{
		...
		{
			// The record has been set. Check the record permissions.
			$permission = $user->authorise('password.edit', 'com_getbible.password.' . (int) $recordId);
			if (!$permission)
			{
				if ($user->authorise('password.edit.own', 'com_getbible.password.' . $recordId))
				{
					...
					if ($ownerId == $user->id)
					{
						if ($user->authorise('password.edit.own', 'com_getbible'))
						{
							return true;
						}
					}
		...
		return $user->authorise('password.edit', $this->option);
	}
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__getbible_password'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_getbible.edit.password.data', []);
		...
		$this->preprocessData('com_getbible.password', $data);
	...
	protected function batchCopy($values, $pks, $contexts)
	{
		if (empty($this->batchSet))
		{
			...
			$this->table 		= $this->getTable();
			$this->tableClassName	= get_class($this->table);
			$this->canDo		= Actions::get('password');
		}

		if (!$this->canDo->get('password.create') && !$this->canDo->get('password.batch'))
		{
			return false;
		}
		...
		elseif (isset($values['published']) && !$this->canDo->get('password.edit.state'))
		{
				$values['published'] = 0;
		}
		...
		while (!empty($pks))
		{
			...
			if (!$this->user->authorise('password.edit', $contexts[$pk]))
			{
				// Not fatal error
	...
	protected function batchMove($values, $pks, $contexts)
	{
		...
		if (!$this->canDo->get('password.edit') && !$this->canDo->get('password.batch'))
		{
		...
		if (isset($values['published']) && !$this->canDo->get('password.edit.state'))
		{
	...
	public function save($data)
	{
		...
		if (empty($data['guid']) && $data['id'] > 0)
		{
			...
			$data['guid'] = (string) GetHelper::var('password', $data['id'], 'id', 'guid');
		}

		// Set the GUID if empty or not valid
		while (!GuidHelper::valid($data['guid'], "password", $data['id']))
		{
			// must always be set
