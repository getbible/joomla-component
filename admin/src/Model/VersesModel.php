...
class VersesModel extends ListModel
{
	...
	public function getItems()
	{
		...
		if (UtilitiesArrayHelper::check($items))
		{
			...
			foreach ($items as $nr => &$item)
			{
				...
				$access = ($user->authorise('verse.access', 'com_getbible.verse.' . (int) $item->id) && $user->authorise('verse.access', 'com_getbible'));
				if (!$access)
				{
					unset($items[$nr]);
					continue;
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__getbible_verse', 'a'));
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('(a.book_nr LIKE '.$search.' OR a.chapter LIKE '.$search.' OR a.verse LIKE '.$search.' OR a.abbreviation LIKE '.$search.' OR g.translation LIKE '.$search.' OR a.name LIKE '.$search.')');
			}
		}
	...
	protected function checkInNow(): void
	{
		...
		if ($time)
		{
			...
			if ($db->getNumRows())
			{
				...
				// Check table.
				$query->update($db->quoteName('#__getbible_verse'))->set($fields)->where($conditions); 
