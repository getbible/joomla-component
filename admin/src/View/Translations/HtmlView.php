...
/------------------------------------------------------------------------------------------------------*/
namespace TrueChristianBible\Component\GetBible\Administrator\View\Translations;
...
class HtmlView extends BaseHtmlView
{
	...
	protected function addToolbar(): void
	{
		...
		if ($this->canCreate)
		{
			$toolbar->addNew('translation.add');
		}
		...
		if (!$this->isEmptyState)
		{
			...
			if ($this->canEdit)
			{
				$childBar->edit('translation.edit')->listCheck(true);
			}

			if ($this->canState)
			{
				$childBar->publish('translations.publish')->listCheck(true);
				$childBar->unpublish('translations.unpublish')->listCheck(true);
				$childBar->archive('translations.archive')->listCheck(true);

				if ($this->canDo->get('core.admin'))
				{
					$childBar->checkin('translations.checkin')->listCheck(true);
				}

				if ($this->state->get('filter.published') == -2 && $this->canDelete)
				{
					$toolbar->delete('translations.delete', 'JTOOLBAR_DELETE_FROM_TRASH')
						->message('JGLOBAL_CONFIRM_DELETE')
						->listCheck(true);
				}
				elseif ($this->canDelete)
				{
					$childBar->trash('translations.trash')->listCheck(true);
				}
			}
		...
		if ($this->user->authorise('translation.remove_duplicate_books', 'com_getbible'))
		{
			// add Remove Duplicate Books button.
			ToolbarHelper::custom('translations.removeDuplicateBooks', 'health custom-button-removeduplicatebooks', '', 'COM_GETBIBLE_REMOVE_DUPLICATE_BOOKS', false);
		}
