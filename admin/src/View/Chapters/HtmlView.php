...
/------------------------------------------------------------------------------------------------------*/
namespace TrueChristianBible\Component\GetBible\Administrator\View\Chapters;
...
class HtmlView extends BaseHtmlView
{
	...
	{
		ToolbarHelper::title(Text::_('COM_GETBIBLE_CHAPTERS'), 'generic');
		/** @var  Toolbar $toolbar */
		$toolbar = $this->getDocument()->getToolbar();
		if ($this->canCreate)
		{
			$toolbar->addNew('chapter.add');
		}
		...
		if (!$this->isEmptyState)
		{
			...
			if ($this->canEdit)
			{
				$childBar->edit('chapter.edit')->listCheck(true);
			}

			if ($this->canState)
			{
				$childBar->publish('chapters.publish')->listCheck(true);
				$childBar->unpublish('chapters.unpublish')->listCheck(true);
				$childBar->archive('chapters.archive')->listCheck(true);

				if ($this->canDo->get('core.admin'))
				{
					$childBar->checkin('chapters.checkin')->listCheck(true);
				}

				if ($this->state->get('filter.published') == -2 && $this->canDelete)
				{
					$toolbar->delete('chapters.delete', 'JTOOLBAR_DELETE_FROM_TRASH')
						->message('JGLOBAL_CONFIRM_DELETE')
						->listCheck(true);
				}
				elseif ($this->canDelete)
				{
					$childBar->trash('chapters.trash')->listCheck(true);
				}
			}
		}
		if ($this->user->authorise('chapter.remove_duplicate_chapters', 'com_getbible'))
		{
			// add Remove Duplicate Chapters button.
			ToolbarHelper::custom('chapters.removeDuplicateChapters', 'health custom-button-removeduplicatechapters', '', 'COM_GETBIBLE_REMOVE_DUPLICATE_CHAPTERS', false);
		}
		if ($this->user->authorise('chapter.remove_duplicate_verses', 'com_getbible'))
		{
			// add Remove Duplicate Verses button.
			ToolbarHelper::custom('chapters.removeDuplicateVerses', 'health custom-button-removeduplicateverses', '', 'COM_GETBIBLE_REMOVE_DUPLICATE_VERSES', false);
		}
	...
	protected function getSortFields()
	{
		return array(
			...
			'a.name' => Text::_('COM_GETBIBLE_CHAPTER_NAME_LABEL'),
			'a.chapter' => Text::_('COM_GETBIBLE_CHAPTER_CHAPTER_LABEL'),
			'a.book_nr' => Text::_('COM_GETBIBLE_CHAPTER_BOOK_NR_LABEL'),
			'g.translation' => Text::_('COM_GETBIBLE_CHAPTER_ABBREVIATION_LABEL'),
