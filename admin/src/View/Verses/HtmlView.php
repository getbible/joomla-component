...
/------------------------------------------------------------------------------------------------------*/
namespace TrueChristianBible\Component\GetBible\Administrator\View\Verses;
...
class HtmlView extends BaseHtmlView
{
	...
	protected function addToolbar(): void
	{
		...
		if (!$this->isEmptyState)
		{
			...
			if ($this->canEdit)
			{
				$childBar->edit('verse.edit')->listCheck(true);
			}

			if ($this->canState)
			{
				$childBar->publish('verses.publish')->listCheck(true);
				$childBar->unpublish('verses.unpublish')->listCheck(true);
				$childBar->archive('verses.archive')->listCheck(true);

				if ($this->canDo->get('core.admin'))
				{
					$childBar->checkin('verses.checkin')->listCheck(true);
				}

				if ($this->state->get('filter.published') == -2 && $this->canDelete)
				{
					$toolbar->delete('verses.delete', 'JTOOLBAR_DELETE_FROM_TRASH')
						->message('JGLOBAL_CONFIRM_DELETE')
						->listCheck(true);
				}
				elseif ($this->canDelete)
				{
					$childBar->trash('verses.trash')->listCheck(true);
				}
			}
		}
		if ($this->user->authorise('verse.remove_duplicate_verses', 'com_getbible'))
		{
			// add Remove Duplicate Verses button.
			ToolbarHelper::custom('verses.removeDuplicateVerses', 'health custom-button-removeduplicateverses', '', 'COM_GETBIBLE_REMOVE_DUPLICATE_VERSES', false);
		}
