...
/------------------------------------------------------------------------------------------------------*/
namespace TrueChristianBible\Component\GetBible\Administrator\View\Open_ai_messages;
...
class HtmlView extends BaseHtmlView
{
	...
	protected function addToolbar(): void
	{
		...
		if ($this->canCreate)
		{
			$toolbar->addNew('open_ai_message.add');
		}
		...
		if (!$this->isEmptyState)
		{
			...
			if ($this->canEdit)
			{
				$childBar->edit('open_ai_message.edit')->listCheck(true);
			}

			if ($this->canState)
			{
				$childBar->publish('open_ai_messages.publish')->listCheck(true);
				$childBar->unpublish('open_ai_messages.unpublish')->listCheck(true);
				$childBar->archive('open_ai_messages.archive')->listCheck(true);

				if ($this->canDo->get('core.admin'))
				{
					$childBar->checkin('open_ai_messages.checkin')->listCheck(true);
				}

				if ($this->state->get('filter.published') == -2 && $this->canDelete)
				{
					$toolbar->delete('open_ai_messages.delete', 'JTOOLBAR_DELETE_FROM_TRASH')
						->message('JGLOBAL_CONFIRM_DELETE')
						->listCheck(true);
				}
				elseif ($this->canDelete)
				{
					$childBar->trash('open_ai_messages.trash')->listCheck(true);
				}
			}
		}
