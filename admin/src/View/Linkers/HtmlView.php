...
class HtmlView extends BaseHtmlView
{
	...
	protected function addToolbar(): void
	{
		...
		if (!$this->isEmptyState)
		{
			...
			if ($this->canEdit)
			{
				$childBar->edit('linker.edit')->listCheck(true);
			}

			if ($this->canState)
			{
				$childBar->publish('linkers.publish')->listCheck(true);
				$childBar->unpublish('linkers.unpublish')->listCheck(true);
				$childBar->archive('linkers.archive')->listCheck(true);

				if ($this->canDo->get('core.admin'))
				{
					$childBar->checkin('linkers.checkin')->listCheck(true);
				}

				if ($this->state->get('filter.published') == -2 && $this->canDelete)
				{
					$toolbar->delete('linkers.delete', 'JTOOLBAR_DELETE_FROM_TRASH')
						->message('JGLOBAL_CONFIRM_DELETE')
						->listCheck(true);
				}
				elseif ($this->canDelete)
				{
					$childBar->trash('linkers.trash')->listCheck(true);
				}
			}
		}
