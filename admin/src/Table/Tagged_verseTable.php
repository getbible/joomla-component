...
class Tagged_verseTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_getbible.tagged_verse';
		...
		parent::__construct('#__getbible_tagged_verse', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Tagged_verse table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New tagged_verse. A tagged_verse created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			if ($table->load(['alias' => $this->alias]) && ($table->id != $this->id || $this->id == 0))
			{
				...
				{
					$this->setError(Text::_('COM_GETBIBLE_TAGGED_VERSE_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
