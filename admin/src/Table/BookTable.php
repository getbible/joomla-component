...
class BookTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_getbible.book';
		...
		parent::__construct('#__getbible_book', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Book table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New book. A book created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			if ($table->load(['alias' => $this->alias]) && ($table->id != $this->id || $this->id == 0))
			{
				...
				{
					$this->setError(Text::_('COM_GETBIBLE_BOOK_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_getbible.book.'.$this->id);
			$this->setRules($rules);
		}
