...
class LinkerTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_getbible.linker';
		...
		parent::__construct('#__getbible_linker', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Linker table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New linker. A linker created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_GETBIBLE_LINKER_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_GETBIBLE_LINKER_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_getbible.linker.'.$this->id);
			$this->setRules($rules);
		}
