...
class TranslationTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_getbible.translation';
		...
		parent::__construct('#__getbible_translation', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Translation table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New translation. A translation created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			if ($table->load(['alias' => $this->alias]) && ($table->id != $this->id || $this->id == 0))
			{
				...
				{
					$this->setError(Text::_('COM_GETBIBLE_TRANSLATION_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_getbible.translation.'.$this->id);
			$this->setRules($rules);
		}
