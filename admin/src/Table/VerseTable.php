...
class VerseTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_getbible.verse';
		...
		parent::__construct('#__getbible_verse', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Verse table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New verse. A verse created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_GETBIBLE_VERSE_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_GETBIBLE_VERSE_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_getbible.verse.'.$this->id);
			$this->setRules($rules);
		}
