...
class ChapterTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_getbible.chapter';
		...
		parent::__construct('#__getbible_chapter', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Chapter table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New chapter. A chapter created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			if ($table->load(['alias' => $this->alias]) && ($table->id != $this->id || $this->id == 0))
			{
				...
				{
					$this->setError(Text::_('COM_GETBIBLE_CHAPTER_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_getbible.chapter.'.$this->id);
			$this->setRules($rules);
		}
