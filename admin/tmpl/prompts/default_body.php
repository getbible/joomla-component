...
<?php foreach ($this->items as $i => $item): ?>
	<?php
		...
		$canDo = Actions::get('prompt', $item, 'prompts');
	?>
	<tr class="row<?php echo $i % 2; ?>">
		<td class="order nowrap center hidden-phone">
		<?php if (!$this->isModal && $canDo->get('prompt.edit.state')): ?>
			<?php
				$iconClass = '';
				if (!$this->saveOrder)
				{
		...
		<?php if (!$this->isModal && $canDo->get('prompt.edit')): ?>
				...
				<?php else: ?>
					...
					<?php else: ?>
						<?php
							...
							$attribs = 'data-content-select data-content-type="com_getbible.prompt"'
								. ' data-id="' . $dataId . '"'
