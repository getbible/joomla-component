...
<?php foreach ($this->items as $i => $item): ?>
	...
	<tr class="row<?php echo $i % 2; ?>">
		...
		<?php if (!$this->isModal && $canDo->get('open_ai_message.edit.state')): ?>
			<?php
				$iconClass = '';
				if (!$this->saveOrder)
				{
		...
		<?php if (!$this->isModal && $canDo->get('open_ai_message.edit')): ?>
				...
				<?php else: ?>
					...
					<?php else: ?>
						<?php
							...
							$itemHtml = '<a href="' . $this->escape($link, false) . '">' . $this->escape($item->role, false) . '</a>';
							$attribs = 'data-content-select data-content-type="com_getbible.open_ai_message"'
								. ' data-id="' . $dataId . '"'
								. ' data-title="' . $this->escape($item->role, false) . '"'
