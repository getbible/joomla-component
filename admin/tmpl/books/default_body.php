...
$edit = "index.php?option=com_getbible&view=books&task=book.edit";
...
<?php foreach ($this->items as $i => $item): ?>
	<?php
		...
		$canDo = Actions::get('book', $item, 'books');
	?>
	<tr class="row<?php echo $i % 2; ?>">
		...
		<?php if (!$this->isModal && $canDo->get('book.edit.state')) : ?>
				<?php if ($item->checked_out) : ?>
					...
					<?php else: ?>
						<?php echo Html::_('jgrid.published', $item->published, $i, 'books.', false, 'cb'); ?>
					<?php endif; ?>
				<?php else: ?>
					<?php echo Html::_('jgrid.published', $item->published, $i, 'books.', true, 'cb'); ?>
				<?php endif; ?>
