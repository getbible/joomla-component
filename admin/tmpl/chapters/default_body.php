...
<?php foreach ($this->items as $i => $item): ?>
	<?php
		...
		$canDo = Actions::get('chapter', $item, 'chapters');
	?>
	<tr class="row<?php echo $i % 2; ?>">
		<td class="order nowrap center hidden-phone">
		<?php if (!$this->isModal && $canDo->get('chapter.edit.state')): ?>
			<?php
				$iconClass = '';
				if (!$this->saveOrder)
				{
		...
		<?php if (!$this->isModal && $canDo->get('chapter.edit')): ?>
				...
				<?php else: ?>
					...
					<?php else: ?>
						<?php
							...
							$attribs = 'data-content-select data-content-type="com_getbible.chapter"'
								. ' data-id="' . $dataId . '"'
