...
<?php foreach ($this->items as $i => $item): ?>
	<?php
		...
		$canDo = Actions::get('linker', $item, 'linkers');
	?>
	<tr class="row<?php echo $i % 2; ?>">
		<td class="order nowrap center hidden-phone">
		<?php if (!$this->isModal && $canDo->get('linker.edit.state')): ?>
			<?php
				$iconClass = '';
				if (!$this->saveOrder)
				{
		...
		<?php if (!$this->isModal && $canDo->get('linker.edit')): ?>
				...
				<?php else: ?>
					...
					<?php else: ?>
						<?php
							...
							$attribs = 'data-content-select data-content-type="com_getbible.linker"'
								. ' data-id="' . $dataId . '"'
