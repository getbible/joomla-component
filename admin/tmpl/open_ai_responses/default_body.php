...
<?php foreach ($this->items as $i => $item): ?>
	...
	<tr class="row<?php echo $i % 2; ?>">
		...
		<?php if (!$this->isModal && $canDo->get('open_ai_response.edit.state')): ?>
			<?php
				$iconClass = '';
				if (!$this->saveOrder)
				{
		...
		<?php if (!$this->isModal && $canDo->get('open_ai_response.edit')): ?>
		...
		<td class="nowrap">
			<div class="name">
				<?php if (!$this->isModal && $canDo->get('open_ai_response.edit')): ?>
					...
					<?php if ($item->checked_out): ?>
						<?php echo Html::_('jgrid.checkedout', $i, $userChkOut->name, $item->checked_out_time, 'open_ai_responses.', $canCheckin); ?>
					<?php endif; ?>
				<?php else: ?>
					<?php if (!$this->isModal): ?>
					...
					<?php else: ?>
						<?php
							...
							$itemHtml = '<a href="' . $this->escape($link, false) . '">' . $this->escape($item->response_id, false) . '</a>';
							$attribs = 'data-content-select data-content-type="com_getbible.open_ai_response"'
								. ' data-id="' . $dataId . '"'
								. ' data-title="' . $this->escape($item->response_id, false) . '"'
		...
		<?php if (!$this->isModal && $canDo->get('open_ai_response.edit.state')) : ?>
				<?php if ($item->checked_out) : ?>
					<?php if ($canCheckin) : ?>
						<?php echo Html::_('jgrid.published', $item->published, $i, 'open_ai_responses.', true, 'cb'); ?>
					<?php else: ?>
						<?php echo Html::_('jgrid.published', $item->published, $i, 'open_ai_responses.', false, 'cb'); ?>
					<?php endif; ?>
				<?php else: ?>
					<?php echo Html::_('jgrid.published', $item->published, $i, 'open_ai_responses.', true, 'cb'); ?>
				<?php endif; ?>
