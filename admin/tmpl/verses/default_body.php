...
$edit = "index.php?option=com_getbible&view=verses&task=verse.edit";
...
<?php foreach ($this->items as $i => $item): ?>
	<?php
		...
		$canDo = GetbibleHelper::getActions('verse',$item,'verses');
	?>
	<tr class="row<?php echo $i % 2; ?>">
		<td class="order nowrap center hidden-phone">
		<?php if (!$this->isModal && $canDo->get('verse.edit.state')): ?>
			<?php
				$iconClass = '';
				if (!$this->saveOrder)
				{
		...
		<?php if (!$this->isModal && $canDo->get('verse.edit')): ?>
		...
		<td class="nowrap">
			<div class="name">
				<?php if (!$this->isModal && $canDo->get('verse.edit')): ?>
					...
					<?php if ($item->checked_out): ?>
						<?php echo Html::_('jgrid.checkedout', $i, $userChkOut->name, $item->checked_out_time, 'verses.', $canCheckin); ?>
					<?php endif; ?>
				<?php else: ?>
					<?php if (!$this->isModal): ?>
					...
					<?php else: ?>
						<?php
							...
							$itemHtml = '<a href="' . $this->escape($link, false) . '">' . $this->escape($item->book_nr, false) . '</a>';
							$attribs = 'data-content-select data-content-type="com_getbible.verse"'
								. ' data-id="' . $dataId . '"'
								. ' data-title="' . $this->escape($item->book_nr, false) . '"'
		...
		<td class="hidden-phone">
			<?php echo $this->escape($item->verse); ?>
		</td>
		<td class="nowrap">
			<div class="name">
		...
		<?php if (!$this->isModal && $canDo->get('verse.edit.state')) : ?>
				<?php if ($item->checked_out) : ?>
					<?php if ($canCheckin) : ?>
						<?php echo Html::_('jgrid.published', $item->published, $i, 'verses.', true, 'cb'); ?>
					<?php else: ?>
						<?php echo Html::_('jgrid.published', $item->published, $i, 'verses.', false, 'cb'); ?>
					<?php endif; ?>
				<?php else: ?>
					<?php echo Html::_('jgrid.published', $item->published, $i, 'verses.', true, 'cb'); ?>
				<?php endif; ?>
