...
class GetbibleTableTag extends Table
{
	...
	function __construct(&$db) 
	{
		...
		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_getbible.tag'));
	}	
	...
	public function store($updateNulls = false)
	{
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('tag', 'GetbibleTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_GETBIBLE_TAG_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
