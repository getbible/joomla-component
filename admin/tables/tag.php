 ...
 * Tags Table class
 */
class GetbibleTableTag extends Table
{
	/**
	...
	{
		parent::__construct('#__getbible_tag', 'id', $db);

		// Adding History Options
		TableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_getbible.tag'));
	}
	...
	public function store($updateNulls = false)
	{
		...
		if (isset($this->alias))
		{
			...
			$table = Table::getInstance('tag', 'GetbibleTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(Text::_('COM_GETBIBLE_TAG_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_GETBIBLE_TAG_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_getbible.tag.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_getbible.tag.'.(int) $this->$k;
	}
