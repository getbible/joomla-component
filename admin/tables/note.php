 ...
 * Notes Table class
 */
class GetbibleTableNote extends Table
{
	/**
	...
	{
		parent::__construct('#__getbible_note', 'id', $db);

		// Adding History Options
		TableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_getbible.note'));
	}
	...
	/**
	 * Overload the store method for the Note table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		if (isset($this->alias))
		{
			...
			$table = Table::getInstance('note', 'GetbibleTable');
			...
			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				...
				{
					$this->setError(Text::_('COM_GETBIBLE_NOTE_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_getbible.note.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_getbible.note.'.(int) $this->$k;
	}
