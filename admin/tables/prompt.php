 ...
 * Prompts Table class
 */
class GetbibleTablePrompt extends Table
{
	/**
	...
	{
		parent::__construct('#__getbible_prompt', 'id', $db);

		// Adding History Options
		TableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_getbible.prompt'));
	}
	...
	/**
	 * Overload the store method for the Prompt table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New prompt. A prompt created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = Table::getInstance('prompt', 'GetbibleTable');
			...
			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				...
				{
					$this->setError(Text::_('COM_GETBIBLE_PROMPT_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_getbible.prompt.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_getbible.prompt.'.(int) $this->$k;
	}
