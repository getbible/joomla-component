...
class GetbibleViewLinkers extends HtmlView
{
	...
	protected function addToolBar()
	{
		...
		ToolbarHelper::title(Text::_('COM_GETBIBLE_LINKERS'), 'joomla');
		FormHelper::addFieldPath(JPATH_COMPONENT . '/models/fields');

		if ($this->canCreate)
		{
			ToolbarHelper::addNew('linker.add');
		}
		...
		if (ArrayHelper::check($this->items))
		{
			if ($this->canEdit)
			{
				ToolbarHelper::editList('linker.edit');
			}

			if ($this->canState)
			{
				ToolbarHelper::publishList('linkers.publish');
				ToolbarHelper::unpublishList('linkers.unpublish');
				ToolbarHelper::archiveList('linkers.archive');

				if ($this->canDo->get('core.admin'))
				{
					ToolbarHelper::checkin('linkers.checkin');
				}
			}
			...
			{
				ToolbarHelper::deleteList('', 'linkers.delete', 'JTOOLBAR_EMPTY_TRASH');
			}
			elseif ($this->canState && $this->canDelete)
			{
				ToolbarHelper::trash('linkers.trash');
			}
		}
		...
		{
			// Set Name Selection
			$this->nameOptions = FormHelper::loadFieldType('linkersfiltername')->options;
			...
			JHtmlBatch_::addListSelection(
				'- Keep Original '.Text::_('COM_GETBIBLE_LINKER_NAME_LABEL').' -',
				'batch[name]',
		...
		if ($this->canBatch && $this->canCreate && $this->canEdit)
		{
			...
			$this->public_tagged_versesOptions = FormHelper::loadFieldType('linkersfilterpublictaggedverses')->options;
			...
			if (ArrayHelper::check($this->public_tagged_versesOptions) &&
				...
				!StringHelper::check($this->public_tagged_versesOptions[0]->value))
			{
				unset($this->public_tagged_versesOptions[0]);
			}
			// Public Tagged Verses Batch Selection
			JHtmlBatch_::addListSelection(
				'- Keep Original '.Text::_('COM_GETBIBLE_LINKER_PUBLIC_TAGGED_VERSES_LABEL').' -',
				'batch[public_tagged_verses]',
				Html::_('select.options', $this->public_tagged_versesOptions, 'value', 'text')
			);
		}
		...
		if ($this->canBatch && $this->canCreate && $this->canEdit)
		{
			...
			$this->public_notesOptions = FormHelper::loadFieldType('linkersfilterpublicnotes')->options;
			// We do some sanitation for Public Notes filter
			if (ArrayHelper::check($this->public_notesOptions) &&
				isset($this->public_notesOptions[0]->value) &&
				!StringHelper::check($this->public_notesOptions[0]->value))
			{
				unset($this->public_notesOptions[0]);
			}
			// Public Notes Batch Selection
			JHtmlBatch_::addListSelection(
				'- Keep Original '.Text::_('COM_GETBIBLE_LINKER_PUBLIC_NOTES_LABEL').' -',
				'batch[public_notes]',
				Html::_('select.options', $this->public_notesOptions, 'value', 'text')
			);
		}
	}
	...
	protected function setDocument()
	{
		...
		$this->document->setTitle(Text::_('COM_GETBIBLE_LINKERS'));
	...
	protected function getSortFields()
	{
		return array(
			...
			'a.name' => Text::_('COM_GETBIBLE_LINKER_NAME_LABEL'),
			'a.public_tagged_verses' => Text::_('COM_GETBIBLE_LINKER_PUBLIC_TAGGED_VERSES_LABEL'),
			'a.public_notes' => Text::_('COM_GETBIBLE_LINKER_PUBLIC_NOTES_LABEL'),
