...
class GetbibleViewPrompts extends HtmlView
{
	...
	protected function addToolBar()
	{
		...
		{
			// Set Name Selection
			$this->nameOptions = FormHelper::loadFieldType('promptsfiltername')->options;
			...
			JHtmlBatch_::addListSelection(
				'- Keep Original '.Text::_('COM_GETBIBLE_PROMPT_NAME_LABEL').' -',
				'batch[name]',
		...
		if ($this->canBatch && $this->canCreate && $this->canEdit)
		{
			...
			$this->cache_behaviourOptions = FormHelper::loadFieldType('promptsfiltercachebehaviour')->options;
			// We do some sanitation for Cache Behaviour filter
			if (ArrayHelper::check($this->cache_behaviourOptions) &&
				isset($this->cache_behaviourOptions[0]->value) &&
				!StringHelper::check($this->cache_behaviourOptions[0]->value))
			{
				unset($this->cache_behaviourOptions[0]);
			}
			// Cache Behaviour Batch Selection
			JHtmlBatch_::addListSelection(
				'- Keep Original '.Text::_('COM_GETBIBLE_PROMPT_CACHE_BEHAVIOUR_LABEL').' -',
				'batch[cache_behaviour]',
				Html::_('select.options', $this->cache_behaviourOptions, 'value', 'text')
			);
		}
		...
		if ($this->canBatch && $this->canCreate && $this->canEdit)
		{
			...
			$this->abbreviationTranslationOptions = FormHelper::loadFieldType('Targettranslations')->options;
			...
			JHtmlBatch_::addListSelection(
				'- Keep Original '.Text::_('COM_GETBIBLE_PROMPT_ABBREVIATION_LABEL').' -',
				'batch[abbreviation]',
		...
		if ($this->canBatch && $this->canCreate && $this->canEdit)
		{
			...
			if (ArrayHelper::check($this->modelOptions) &&
				isset($this->modelOptions[0]->value) &&
				!StringHelper::check($this->modelOptions[0]->value))
			{
				unset($this->modelOptions[0]);
			}
			// Model Batch Selection
			JHtmlBatch_::addListSelection(
				'- Keep Original '.Text::_('COM_GETBIBLE_PROMPT_MODEL_LABEL').' -',
				'batch[model]',
				Html::_('select.options', $this->modelOptions, 'value', 'text')
			);
		}
	}
	...
	protected function setDocument()
	{
		...
		$this->document->setTitle(Text::_('COM_GETBIBLE_PROMPTS'));
