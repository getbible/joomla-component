...
class GetbibleViewNotes extends HtmlView
{
	...
	protected function addToolBar()
	{
		...
		ToolbarHelper::title(Text::_('COM_GETBIBLE_NOTES'), 'file');
		...
		if (ArrayHelper::check($this->items))
		{
			if ($this->canEdit)
			{
				ToolbarHelper::editList('note.edit');
			}

			if ($this->canState)
			{
				ToolbarHelper::publishList('notes.publish');
				ToolbarHelper::unpublishList('notes.unpublish');
			...
			{
				ToolbarHelper::deleteList('', 'notes.delete', 'JTOOLBAR_EMPTY_TRASH');
			}
		...
		if ($this->canBatch && $this->canCreate && $this->canEdit)
		{
			...
			$this->book_nrOptions = FormHelper::loadFieldType('notesfilterbooknr')->options;
		...
		if ($this->canBatch && $this->canCreate && $this->canEdit)
		{
			...
			JHtmlBatch_::addListSelection(
				'- Keep Original '.Text::_('COM_GETBIBLE_NOTE_LINKER_LABEL').' -',
				'batch[linker]',
		...
		if ($this->canBatch && $this->canCreate && $this->canEdit)
		{
			...
			$this->accessOptions = FormHelper::loadFieldType('notesfilteraccess')->options;
			...
			JHtmlBatch_::addListSelection(
				'- Keep Original '.Text::_('COM_GETBIBLE_NOTE_ACCESS_LABEL').' -',
				'batch[access]',
		...
		{
			// Set Verse Selection
			$this->verseOptions = FormHelper::loadFieldType('notesfilterverse')->options;
		...
		if ($this->canBatch && $this->canCreate && $this->canEdit)
		{
			...
			$this->chapterOptions = FormHelper::loadFieldType('notesfilterchapter')->options;
			...
			JHtmlBatch_::addListSelection(
				'- Keep Original '.Text::_('COM_GETBIBLE_NOTE_CHAPTER_LABEL').' -',
				'batch[chapter]',
	...
	protected function setDocument()
	{
		...
		$this->document->setTitle(Text::_('COM_GETBIBLE_NOTES'));
	...
	protected function getSortFields()
	{
		return array(
			...
			'a.book_nr' => Text::_('COM_GETBIBLE_NOTE_BOOK_NR_LABEL'),
			'g.name' => Text::_('COM_GETBIBLE_NOTE_LINKER_LABEL'),
			'a.access' => Text::_('COM_GETBIBLE_NOTE_ACCESS_LABEL'),
