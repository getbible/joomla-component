 ...
 */
class GetbibleModelOpen_ai_message extends AdminModel
{
	/**
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_getbible.open_ai_message';
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'open_ai_message', $prefix = 'GetbibleTable', $config = array())
	{
	...
	public function getForm($data = array(), $loadData = true, $options = array('control' => 'jform'))
	{
		...
		// Get the form.
		$form = $this->loadForm('com_getbible.open_ai_message', 'open_ai_message', $options, $clear, $xpath);
		...
		if ($id != 0 && (!$user->authorise('open_ai_message.edit.state', 'com_getbible.open_ai_message.' . (int) $id))
			|| ($id == 0 && !$user->authorise('open_ai_message.edit.state', 'com_getbible')))
		{
		...
		if ($id != 0 && (!$user->authorise('open_ai_message.edit.created_by', 'com_getbible.open_ai_message.' . (int) $id))
			|| ($id == 0 && !$user->authorise('open_ai_message.edit.created_by', 'com_getbible')))
		{
			// Disable fields for display.
		...
		if ($id != 0 && (!$user->authorise('open_ai_message.edit.created', 'com_getbible.open_ai_message.' . (int) $id))
			|| ($id == 0 && !$user->authorise('open_ai_message.edit.created', 'com_getbible')))
		{
			// Disable fields for display.
		...
		// Modify the form based on Edit Role access controls.
		...
		// Modify the form based on Edit Open Ai Response access controls.
		...
		// Modify the form based on Edit Content access controls.
		...
		// Modify the form based on Edit Index access controls.
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('open_ai_message.delete', 'com_getbible.open_ai_message.' . (int) $record->id);
		}
		return false;
	}
	...
	protected function canEditState($record)
	{
		...
		{
			// The record has been set. Check the record permissions.
			$permission = $user->authorise('open_ai_message.edit.state', 'com_getbible.open_ai_message.' . (int) $recordId);
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__getbible_open_ai_message'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_getbible.edit.open_ai_message.data', array());

		if (empty($data))
		{
			$data = $this->getItem();
			// run the perprocess of the data
			$this->preprocessData('com_getbible.open_ai_message', $data);
		}
	...
	protected function batchCopy($values, $pks, $contexts)
	{
		...
		elseif (isset($values['published']) && !$this->canDo->get('open_ai_message.edit.state'))
		{
				$values['published'] = 0;
		}
		...
		while (!empty($pks))
		{
			...
			if (!$this->user->authorise('open_ai_message.edit', $contexts[$pk]))
			{
				// Not fatal error
	...
	protected function batchMove($values, $pks, $contexts)
	{
		...
		if (isset($values['published']) && !$this->canDo->get('open_ai_message.edit.state'))
		{
