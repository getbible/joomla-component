 ...
 */
class JFormFieldTagers extends JFormFieldList
{
	/**
	...
	protected function getInput()
	{
		...
		if ($set_button === 'true')
		{
			...
			if ($user->authorise('tag.create', 'com_getbible') && $app->isAdmin()) // TODO for now only in admin area.
			{
				// build Create button
				$button[] = '<a id="'.$button_code_name.'Create" class="btn btn-small btn-success hasTooltip" title="'.JText::sprintf('COM_GETBIBLE_CREATE_NEW_S', $button_label).'" style="border-radius: 0px 4px 4px 0px; padding: 4px 4px 4px 7px;"
					href="index.php?option=com_getbible&amp;view=tag&amp;layout=edit'.$ref.'" >
					<span class="icon-new icon-white"></span></a>';
			}
			// only add if user allowed to edit tag
			if ($user->authorise('tag.edit', 'com_getbible') && $app->isAdmin()) // TODO for now only in admin area.
			{
				// build edit button
				...
				$script[] = "
					...
					function ".$button_code_name."Button(value) {
						if (value > 0) {
							...
							var url = 'index.php?option=com_getbible&view=tags&task=tag.edit&id='+value+'".$refJ."';
