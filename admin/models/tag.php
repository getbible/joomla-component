 ...
 */
class GetbibleModelTag extends AdminModel
{
	/**
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_getbible.tag';
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'tag', $prefix = 'GetbibleTable', $config = array())
	{
	...
	public function getForm($data = array(), $loadData = true, $options = array('control' => 'jform'))
	{
		...
		// Get the form.
		$form = $this->loadForm('com_getbible.tag', 'tag', $options, $clear, $xpath);
		...
		if ($id != 0 && (!$user->authorise('tag.edit.state', 'com_getbible.tag.' . (int) $id))
			|| ($id == 0 && !$user->authorise('tag.edit.state', 'com_getbible')))
		{
		...
		if ($id != 0 && (!$user->authorise('tag.edit.created_by', 'com_getbible.tag.' . (int) $id))
			|| ($id == 0 && !$user->authorise('tag.edit.created_by', 'com_getbible')))
		{
			// Disable fields for display.
		...
		if ($id != 0 && (!$user->authorise('tag.edit.created', 'com_getbible.tag.' . (int) $id))
			|| ($id == 0 && !$user->authorise('tag.edit.created', 'com_getbible')))
		{
			// Disable fields for display.
		...
		if ($id != 0 && (!$user->authorise('tag.edit.access', 'com_getbible.tag.' . (int) $id))
			|| ($id == 0 && !$user->authorise('tag.edit.access', 'com_getbible')))
		{
			// Disable fields for display.
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('tag.delete', 'com_getbible.tag.' . (int) $record->id);
		}
		return false;
	}
	...
	protected function canEditState($record)
	{
		...
		{
			// The record has been set. Check the record permissions.
			$permission = $user->authorise('tag.edit.state', 'com_getbible.tag.' . (int) $recordId);
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__getbible_tag'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_getbible.edit.tag.data', array());

		if (empty($data))
		{
			$data = $this->getItem();
			// run the perprocess of the data
			$this->preprocessData('com_getbible.tag', $data);
		}
