 ...
 */
class GetbibleModelVerse extends AdminModel
{
	/**
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_getbible.verse';
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'verse', $prefix = 'GetbibleTable', $config = array())
	{
	...
	public function getForm($data = array(), $loadData = true, $options = array('control' => 'jform'))
	{
		...
		// Get the form.
		$form = $this->loadForm('com_getbible.verse', 'verse', $options, $clear, $xpath);
		...
		if ($id != 0 && (!$user->authorise('verse.edit.state', 'com_getbible.verse.' . (int) $id))
			|| ($id == 0 && !$user->authorise('verse.edit.state', 'com_getbible')))
		{
		...
		if ($id != 0 && (!$user->authorise('verse.edit.created_by', 'com_getbible.verse.' . (int) $id))
			|| ($id == 0 && !$user->authorise('verse.edit.created_by', 'com_getbible')))
		{
			// Disable fields for display.
		...
		if ($id != 0 && (!$user->authorise('verse.edit.created', 'com_getbible.verse.' . (int) $id))
			|| ($id == 0 && !$user->authorise('verse.edit.created', 'com_getbible')))
		{
			// Disable fields for display.
		...
		// Modify the form based on Edit Text access controls.
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('verse.delete', 'com_getbible.verse.' . (int) $record->id);
		}
		return false;
	}
	...
	protected function canEditState($record)
	{
		...
		{
			// The record has been set. Check the record permissions.
			$permission = $user->authorise('verse.edit.state', 'com_getbible.verse.' . (int) $recordId);
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__getbible_verse'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_getbible.edit.verse.data', array());

		if (empty($data))
		{
			$data = $this->getItem();
			// run the perprocess of the data
			$this->preprocessData('com_getbible.verse', $data);
		}
	...
	protected function batchCopy($values, $pks, $contexts)
	{
		...
		elseif (isset($values['published']) && !$this->canDo->get('verse.edit.state'))
		{
				$values['published'] = 0;
		}
		...
		while (!empty($pks))
		{
			...
			if (!$this->user->authorise('verse.edit', $contexts[$pk]))
			{
				// Not fatal error
	...
	protected function batchMove($values, $pks, $contexts)
	{
		...
		if (isset($values['published']) && !$this->canDo->get('verse.edit.state'))
		{
