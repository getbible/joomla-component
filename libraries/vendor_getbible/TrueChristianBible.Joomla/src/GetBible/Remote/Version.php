...
namespace TrueChristianBible\Joomla\GetBible\Remote;


use Joomla\CMS\Language\Text;
use TrueChristianBible\Joomla\Github\Factory as Github;
use TrueChristianBible\Joomla\Gitea\Factory as Gitea;
...
final class Version
{
	...
	{
		$defaultDownloadLink = '[[[component_package_download_link]]]';
		...
		if ($versionType !== 'stable')
		{
			...
			$state = $isLatestPre ? Text::_('COM_GETBIBLE_PRE_RELEASE') : Text::_('COM_GETBIBLE_OUT_OF_DATE');
			$statement = sprintf(
			...
			$state = $isLatestPre
				? Text::sprintf("COM_GETBIBLE_THANK_YOU_FOR_TESTING_THE_S_RELEASE_YOURE_A_JCB_HERO", strtoupper($versionType))
				: Text::sprintf("COM_GETBIBLE_GRAB_THE_LATEST_S_TESTING_RELEASE", strtoupper($versionType));
		...
		if ($versionCompare === 0)
		{
			$notice = sprintf(
				...
				Text::_('COM_GETBIBLE_UP_TO_DATE')
			);
			...
			if (!empty($grouped['pre']))
			{
				$notice = sprintf(
					...
					Text::_('COM_GETBIBLE_HELP_US_TEST_THE_UPCOMING_RELEASE'),
					$notice
				);
			}
		...
		return ['notice' => sprintf(
			...
			$latestStableLink,
			Text::_('COM_GETBIBLE_YOU_CAN_DIRECTLY_DOWNLOAD_THE_LATEST_UPDATE_OR_USE_THE_JOOMLA_UPDATE_AREA'),
			Text::_('COM_GETBIBLE_DOWNLOAD_UPDATE') . '!'
		)];
	}
